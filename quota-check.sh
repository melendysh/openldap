#!/bin/sh
# Reports Quota Info by Shawn Melendy
/usr/sbin/repquota /cpsc|grep -v -- "--" |sort -k3 -n |grep 0 > /scripts/tmp/quotareport
# repquota /cpsc|grep -v -- "--" |sort -k3 -n |grep 0 |awk {'print $1'}| xargs mailx -s "Your personal file space in the Linux lab is near capacity limits"  
